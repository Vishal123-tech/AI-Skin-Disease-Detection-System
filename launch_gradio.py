"""Launcher that prints Gradio share URL and saves it to public_url.txt."""
import sys
import os
...
> **Disclaimer:** Educational screening prototype only. This result is not a medical diagnosis."""
...
def run_self_training_ui() -> str:
    try:
        ...
        if res.get("success"):
            ...
            return (
                f"### 🚀 Self-Training Completed!\n\n"
                f"- **Summary:** {res.get('message')}\n"
                f"- **Classes ({len(res.get('classes', []))}):** {classes_str}\n"
                f"- **Validation Accuracy:** {res.get('val_accuracy', 0):.1%}\n"
                f"- **Active Backend:** `{predictor.backend}`\n"
                f"> **Live Update:** Model hot-reloaded into memory."
            )
        else:
...
with gr.Blocks(title="AI Skin Disease Detection") as demo:
    gr.Markdown(
        ...
        "Upload a skin image to run the classifier and generate a PDF report."
    )
    gr.Markdown("> **Important:** Educational prototype only — not a medical device.")
    with gr.Row():
    ...
    with gr.Group(visible=False) as feedback_group:
        gr.Markdown("### Help improve this prototype\nWas the result correct?")
        feedback_rating = gr.Radio(["Correct", "Wrong", "Not sure"], label="Prediction feedback", value="Correct")
        correction_label = gr.Dropdown(choices=available_feedback_labels(), label="What is the correct condition?", visible=False)
        feedback_comment = gr.Textbox(label="Optional note", lines=2)
        feedback_consent = gr.Checkbox(label="I consent to storing this image for supervised model improvement.", value=False)
        feedback_button = gr.Button("Save Feedback")
        feedback_status = gr.Markdown()
    with gr.Accordion("🧠 Continuous Self-Training", open=True):
    ...
    analyze_button.click(analyze, inputs=image, outputs=[result, report_file, feedback_context, feedback_group])
    feedback_rating.change(lambda r: gr.update(visible=r == "Wrong"), inputs=feedback_rating, outputs=correction_label)
    feedback_button.click(submit_feedback, inputs=[feedback_context, feedback_rating, correction_label, feedback_comment, feedback_consent], outputs=feedback_status).then(refresh_stats_ui, outputs=stats_md)
    train_btn.click(run_self_training_ui, outputs=train_status).then(refresh_stats_ui, outputs=stats_md)
    refresh_btn.click(refresh_stats_ui, outputs=stats_md)

if __name__ == "__main__":
    print("=== LAUNCHING GRADIO ===", flush=True)
    result_tuple = demo.launch(server_name="0.0.0.0", server_port=PORT, share=True, prevent_thread_lock=True)
    local_url = result_tuple[1]
    share_url = result_tuple[2]
    ...
    if share_url:
        ...
        print(f"Saved to public_url.txt: {share_url}", flush=True)
    else:
        print("WARNING: share_url is None - Gradio tunneling may have failed.", flush=True)
    # Keep alive
    demo.block_thread()
